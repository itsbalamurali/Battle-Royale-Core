...
public class BRS_ChangeCircle : MonoBehaviour
{
	...
	[Range(10, 100)]
  public int ZoneRadiusFactor = 75; //default to 50%
  ...
  public List<int> ZoneTimes;
	...
	private int timeToShrink = 100; //seconds
	private int count = 0;
	...
	void Update ()
	{
		ZoneWall.transform.localScale = new Vector3 ((XRadius * 0.01f), 60, (XRadius * 0.01f));
...
Debug.Log("New Center Point is " + centerPoint);
			...
			XRadius = Mathf.MoveTowards(XRadius, shrinkRadius, (shrinkRadius / timeToShrink) * Time.deltaTime * -1);
